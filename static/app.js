...
function orchestratorInspectorProgress(p,o){
  ...
  const purpose=latestOrchestratorPurpose(o),title=orchestratorPurposeLabels[purpose]||(p.status==='planning'?'Analyzing the mission and workspace':'Continuing the orchestrator turn'),thread=o.thread_id||p.orchestrator_thread_id||'';
  return `<div class="orchestrator-live"><i></i><div><span>ORCHESTRATOR IS WORKING</span><b>${esc(title)}</b><small>${thread?`Same conversation · ${esc(short(thread,24))}`:'Starting the mission conversation'} · live activity will appear below</small></div></div>`;
}
function activityEvent(ev){
  ...
  if(type==='agentdock.orchestrator_turn'){const purpose=orchestratorPurposeLabels[o.purpose]||'Orchestrator turn';return `<div class="activity-milestone ${o.status==='failed'?'failed':''}"><span>Orchestrator</span><b>${esc(purpose)}</b><small>${esc(statusLabel(o.status||'completed'))}${o.thread_id?` · conversation ${esc(short(o.thread_id,20))}`:''}</small></div>`}
  if(type==='thread.started'||type==='thread/started'){const who=String(inspectorTaskId||'').startsWith('orchestrator:')?'Orchestrator':'Agent';return `<div class="activity-milestone"><span>Conversation</span><b>${who} session ready</b><small>${esc(short(o.thread_id||params.thread?.id||params.threadId||'',32))}</small></div>`}
...
async function refreshInspector(fetchDiff=true){
  ...
  if(isRoot){
    ...
    $('taskContract').innerHTML=`<section><h4>Mission</h4><p>${esc(p.goal)}</p></section><section><h4>Decision</h4><p>${esc(decisionLabel(p.decision))}</p><p>${esc(p.decision_reason||'')}</p></section>${p.final_response?`<section><h4>Result</h4><p>${esc(p.final_response)}</p></section>`:''}${contractList('Evidence',evidence)}${contractList('Questions',questions)}<section><h4>Conversation</h4><p>Talk with the orchestrator in this mission's existing conversation. Worker assignments remain editable in Plan Review before execution starts.</p></section>`;
    $('inspectStop').style.display='none';$('openTerminalBtn').style.display='none';document.querySelector('.manual-control').style.display='block';$('manualSend').disabled=false;$('manualSend').textContent='Send ↵';
    ...
    try{
      const [e,l]=await Promise.all([api('/api/events/'+inspectorTaskId),api('/api/logs/'+inspectorTaskId)]);
      const rendered=normalizeEvents(e.events||[]).map(activityEvent).filter(Boolean).join('');
      const progress=orchestratorInspectorProgress(p,live.orchestrator||{});
      $('activityFeed').innerHTML=progress+(rendered||'<div class="empty-state">No readable orchestrator events yet.</div>');
      $('inspectorRaw').textContent=l.logs.map(x=>`[${x.stream}] ${x.line}`).join('\n');
      $('diffText').textContent='Orchestrator runs in the control plane; inspect worker diffs on their task cards.';
    }catch(err){$('activityFeed').innerHTML=`<div class="activity-error">${esc(err.message)}</div>`}
    return
  }
