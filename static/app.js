...
function renderWorkspaceNav(){
  $('workspaceTree').innerHTML=(state.workspaces||[]).map(w=>{
    const plans=(state.plans||[]).filter(p=>p.workspace_id===w.id), running=plans.filter(p=>['running','planning','preflight','awaiting_apply','waiting_for_user','pausing','resuming'].includes(p.status)).length, attention=plans.filter(p=>['attention','blocked','paused'].includes(p.status)).length;
...
function overviewWorkspaceCard(w){
  const plans=(state.plans||[]).filter(p=>p.workspace_id===w.id),active=plans.filter(p=>['running','planning','preflight','waiting_for_user','pausing','resuming'].includes(p.status)).length,attention=plans.filter(p=>['attention','blocked','paused'].includes(p.status)).length;
...
function renderGlobalMissions(){
  ...
  if(statusFilter==='active')plans=plans.filter(p=>['running','planning','preflight','planned','approved','awaiting_apply','waiting_for_user','pausing','paused','resuming'].includes(p.status));
  else if(statusFilter==='attention')plans=plans.filter(p=>['attention','failed','blocked','waiting_for_user','paused'].includes(p.status));
  else if(statusFilter==='waiting_for_user')plans=plans.filter(p=>p.status==='waiting_for_user');
...
function missionActions(p,tasks){
  ...
  else if(p.status==='attention'||p.status==='failed')primary=`<button class="primary compact start-btn" onclick="event.stopPropagation();resumePlan('${p.id}')">Resume from issue ↵</button>`;
...
function renderOrchestrator(o){
  ...
  const isWorking=['planning','preflight','running','pausing','resuming'].includes(p.status);
  ...
  const working=isWorking?`<div class="classic-working"><i></i><b>${p.status==='resuming'?'Continuing':p.status==='pausing'?'Pausing':'Working'}</b><span>(${elapsed(p.started_at||p.created_at,null,live.server_time)} · click to inspect)</span></div>`:'';
...
function renderHistory(){const plans=(state.plans||[]).filter(p=>!selectedWorkspaceId||p.workspace_id===selectedWorkspaceId);$('planHistory').innerHTML=plans.slice(0,8).map((p,i)=>`<button class="history-chip ${p.id===selectedPlanId?'active':''}" onclick="selectPlan('${p.id}')">${i===0?'latest · ':''}${esc(short(missionTitle(p),28))}</button>`).join('')}
...
function orchestratorInspectorProgress(p,o){
  if((o.turn_status||p.orchestrator_turn_status)!=='running')return '';
  ...
  return `<div class="orchestrator-live"><i></i><div><span>ORCHESTRATOR IS WORKING</span><b>${esc(title)}</b><small>Same mission conversation · live activity will appear below</small></div></div>`;
}
